...
genus_alpha_div.meta <- genus_alpha_div %>% rownames_to_column(var = "ID") %>% 
  left_join(., meta.tab.filt %>% dplyr::select(ID, host_clinical_state, host_fev1, Host_age, isolate), by = "ID")
...
observed_me <- mediation_analysis_for_alpha_diversity(X, Y, COV, grp, alpha_diversity=observed_genera, n_boot = 2000, n_cores = parallel::detectCores()-1)
shannon_me <- mediation_analysis_for_alpha_diversity(X, Y, COV, grp, alpha_diversity=shannon, n_boot = 2000, n_cores = parallel::detectCores()-1)
simpson_me <- mediation_analysis_for_alpha_diversity(X, Y, COV, grp, alpha_diversity=simpson, n_boot = 2000, n_cores = parallel::detectCores()-1)
